...
export function jsonToPlainText(json, title = 'Main Group', prefix = '', isRoot = true) {
  ...
  let arrayGroups = '';
  const subGroupPrefix = prefix + '  '; // Manage indentation for sub-groups
  ...
  Object.keys(json).forEach((key) => {
    ...
    if (Array.isArray(value)) {
      ...
      const arrayTitle = `${key.toUpperCase()} GROUP`;
      arrayGroups += `${prefix}${arrayTitle}:\n`;
      value.forEach((item, idx) => {
        const itemPrefix = `${subGroupPrefix}${arrayTitle} [${idx}]:\n`;
        if (typeof item === 'object' && item !== null) {
          // If the item is an object, recurse to print its properties with an index
          arrayGroups += itemPrefix + jsonToPlainText(item, title, subGroupPrefix + '  ', false);
        } else {
      ...
      result += `${prefix}${key}:\n` + jsonToPlainText(value, title, subGroupPrefix, false);
    } else {
